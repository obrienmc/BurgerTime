...
public class Level {
	 ...
	 * @param score 
	 */
	public Level(DrawableComponent draw,Score score) {
		this.draw = draw;
		this.currentLevel = 1;
	...
	public void upLevel() {
		...
		if (this.currentLevel != 4) {
			this.draw.isLevelThreading[this.currentLevel - 1] = false;
			this.currentLevel++;
			this.loadLevel();
			this.draw.initialize();
		} else {
			this.draw.hero.lives = -1;
			this.draw.game.gameOver(false);
	...
	public Burger getBurger(int index) {
		...
		return new Burger(burgerPosition, barPosition,this.score);
	}
	...
	public Guard getGuard(int index) {
		...
		return new Guard(new Point2D.Double(this.guards[index].getX(),
				...
				this.guards[index].getHeight(), this.getPath(), this.draw, this.currentLevel - 1,this.score);
	}
}
