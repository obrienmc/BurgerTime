 ...
 * 
 * @author xiaox. Created Feb 17, 2014.
 */
public class Score {
	...
	public Score() {
		...
		while (this.in.hasNextLine()) {
			...
			this.highScore.add(Integer.parseInt(line.substring(i + 1,
					line.length())));
		...
		if (this.highScore.size() > 0) {
			this.highScoreLabel = new JLabel("  Highest Score  :  "
					+ this.highScore.get(0) + "  ");
		} else {
	...
	public int getHighScore(int index) {
		if (index >= this.highScore.size()) {
			return 0;
		}
		return this.highScore.get(index);
	}
	...
	public String getHighScoreName(int index) {
		if (index >= this.highScoreName.size()) {
			return "-";
		}
		return this.highScoreName.get(index);
	}
	...
	public void printOut() {
		...
		for (int i = 0; i < this.highScore.size(); i++) {
			...
			this.outputFile.println(this.highScoreName.get(i) + " "
					+ this.highScore.get(i));
